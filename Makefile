...
build-image: $(CONFIG_DOCKER_TARGET) build
	...
	@docker build ${IMAGE_BUILD_OPTS} -t $(IMAGE_REPO)/$(IMAGE_NAME)-$(LOCAL_ARCH):$(VERSION) -f build/Dockerfile .
...
install: ## Install all resources (CR/CRD's, RBCA and Operator)
	...
	- cat deploy/olm-catalog/${BASE_DIR}/${CSV_VERSION}/${BASE_DIR}.v${CSV_VERSION}.clusterserviceversion.yaml \
		| sed -e "s|image: quay.io/opencloudio/ibm-healthcheck-operator:latest|image: $(IMAGE_REPO)/$(IMAGE_NAME)-$(LOCAL_ARCH):$(VERSION)|g" \
